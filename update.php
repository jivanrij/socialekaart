...
function update_script_selection_form($form, &$form_state) {
    ...
    foreach (update_resolve_dependencies($starting_updates) as $function => $data) {
        if (!$data['allowed']) {
            ...
            if (isset($form['start'][$module_update_key]['#items'][$data['number']])) {
                $text = $data['missing_dependencies'] ? 'This update will been skipped due to the following missing dependencies: <em>' . implode(', ',
                        $data['missing_dependencies']) . '</em>' : "This update will be skipped due to an error in the module's code.";
    ...
    if ($incompatible_updates_exist) {
        drupal_set_message('Some of the pending updates cannot be applied because their dependencies were not met.',
            'warning');
    }
    ...
    if (empty($count)) {
        ...
        $form['links'] = array(
            '#markup' => theme('item_list',
                array('items' => update_helpful_links())),
        );
        ...
        if ($incompatible_count) {
            ...
            $form['start']['#title'] = format_plural($count, '1 pending update',
...
function update_task_list($active = NULL) {
    ...
    drupal_add_region_content('sidebar_first',
        theme('task_list', array('items' => $tasks, 'active' => $active)));
}
...
function update_check_requirements($skip_warnings = FALSE) {
    ...
    if ($severity == REQUIREMENT_ERROR || ($severity == REQUIREMENT_WARNING && !$skip_warnings)) {
        ...
        $status_report = theme('status_report',
...
if (isset($output) && $output) {
    ...
    print theme('update_page',
        array('content' => $output, 'show_messages' => !$progress_page));
}
