...
//Een checksum. Deze bestaat uit Transactie ID + Transactie Code + Status + Salt
    ...
    if (isset($_GET['id']) && isset($_GET['status']) && isset($_GET['salt']) && isset($_GET['checksum'])) {
        ...
        $oInfo = db_query("SELECT description, bank, payed, ideal_id, ideal_code, gid, uid FROM {gojira_payments} WHERE ideal_id = :id", array(':id' => $_GET['id']))->fetchObject();
        if ($oInfo) {
            ...
            if ($bStatus && $bDetails) {
                ...
                db_query("UPDATE {gojira_payments} SET `status`=1, `increment`=:increment WHERE `ideal_id`=:id AND `ideal_code`=:code ", array(':id' => $oInfo->ideal_id, ':code' => $oInfo->ideal_code, ':increment' => $iIncrement));
                Subscriptions::subscribe($oInfo->ideal_id);
                header('Location: /idealsuccess?id=' . $_GET['id']);
                exit;
            } else {
                watchdog(GojiraSettings::WATCHDOG_IDEAL, 'ideal return: payment fail ideal_id: ' . $_GET['id'] . '<br /> lastError: ' . $oQantani->getLastError());
